    ...
    <section ref={drop} className="album">
        ...
        {albums.map((album) => (
          <li key={album.id}>
            <div className="thumbnail-img">
              <img width="50" height="50" src={album.url} alt={album.title} />
            </div>
            <div className="thumbnail-close" onClick={() => handleDelete(album)}>
      ...
      <div className="album-list">
        {albums.map((album, index) => (
          <div key={album.id} className="thumbnail-title">
            {`${index + 1}. `}
            {album.title}
          </div>
        ))}
